...
public class TMP extends DomainConcept {

  protected static final Logger logWriter = Logger.getLogger(TMP.class.getName());
  ...
  protected ArrayList<Instance> hasInstances = new ArrayList<Instance>(0);

  public void setHasInstancesRawValueInternal(String rawVal) {
  }
  ...
  public TMP(boolean fromDB) {
    super(Workspace.getCurrentWorkspace(), fromDB);
  }
  public TMP(Workspace workspace, boolean fromDB) {
    super(workspace, fromDB);
  }
  public int getId() {
    return id;
  }
  public boolean hasInstancesAllResident() {
      return true;
  }

  /** returns true if this object contains the given object without accessing the database **/
  public boolean containsHasInstances(Instance value) {
      return hasInstances.contains(value);
  }

  public ArrayList<Instance> getHasInstances() {
      return hasInstances;
  }
  ...
  public void setId (int a_id) {
      if (a_id> -1) {
        id = a_id;
        TMPManager.getInstance().putInCache(this);
      }
  }

  /** note that this method does not check for duplicates, which may cause a DB error */
  public void setHasInstances(Collection<Instance> newHasInstances) {
    hasInstances=new ArrayList<Instance>(newHasInstances);
  }

  public void add1HasInstances(Instance newHasInstances) {
    hasInstances.add(newHasInstances);
  }

  /** note that this method does not check for duplicates, which may cause a DB error */
  public void addAllHasInstances(Collection<Instance> newHasInstances) {
    hasInstances.addAll(newHasInstances);
  }

  public boolean remove1HasInstances(Instance newHasInstances) {
    return hasInstances.remove(newHasInstances);
  }
  ...
  public boolean equals(Object o) {
    if (!(o instanceof TMP)) //covers o == null case
      return false;
    TMP other = (TMP)o;
    if (this == other)
      return true;
